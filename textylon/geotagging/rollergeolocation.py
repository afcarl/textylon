...
def user_info(write_user_info=False):
    ...
    with codecs.open(trainfile, 'w', encoding) as trainout:
        with codecs.open(testfile, 'w', encoding) as testout:
            for f in (train, test):
                print "reading " + f
                with codecs.open(f, 'r', encoding) as trainf:
                    l = 0
                    ll = 0
                    ...
                    for line in trainf:
                        ...
                        if f == train:
                            if user not in trainUsers:
                                ...
                                if write_user_info:
                                    trainout.write(user + '\t' + lat + '\t' + lon + '\n')
                        elif f == test:
                            if user not in testUsers:
                                testUsers[user] = locStr
                                if write_user_info:
                                    testout.write(user + '\t' + lat + '\t' + lon + '\n')
                        
                    print "format error count: " + str(l)
                    print "coordinate error count: " + str(coordinateErrorCount)
                    print "total number of lines: " + str(ll)

        
    

def readRollerRecords():
    with codecs.open(path.join(GEOTEXT_HOME, 'full_text.txt'), 'r', encoding) as inf:
        i = 0
        j = 0
        for line in inf:
            i += 1
...
def fillTextByUser(writeUserTexts=False):
        ...
        if writeUserTexts:
            for user in userText:
                with codecs.open(path.join(userTextDirectory, user), 'w', encoding) as outf:
...
def createTrainDir(granularity=10):
    ...
    with codecs.open(filename, 'r', encoding) as inf:
        for line in inf:
            ...
            allpointsMinLon.append(minlon)
            allpoints.append(points)
    trainhome = filename.replace(".train", "")
